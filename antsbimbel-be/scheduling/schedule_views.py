...
from .api_parameters import (
    ...
    SCHEDULE_LIST_QUERY_PARAMETERS,
)
...
class EmailBlastPayloadSerializer(serializers.Serializer):
    mode = serializers.ChoiceField(
        choices=[EmailBlastRecord.TYPE_DAILY, EmailBlastRecord.TYPE_WEEKLY]
    )
...
class ScheduleViewSet(viewsets.ModelViewSet):
    queryset = (
        Schedule.objects.select_related("tutor", "check_in", "check_in__check_out")
        .all()
    ...
    def _validate_schedule_window(start_datetime, end_datetime):
        ...
        if (
            timezone.localtime(start_datetime).date()
            != timezone.localtime(end_datetime).date()
        ):
            raise ValidationError(
                {
        ...
        if schedule_duration < ScheduleViewSet.MINIMUM_SCHEDULE_DURATION:
            raise ValidationError(
                {"end_datetime": "Schedule duration must be at least 2 hours."}
            )

        if schedule_duration % ScheduleViewSet.SESSION_DURATION != timedelta(0):
            raise ValidationError(
                {"end_datetime": "Schedule duration must be in multiples of 2 hours."}
            )

        if schedule_duration != ScheduleViewSet.SESSION_DURATION:
            raise ValidationError(
                {"end_datetime": "Schedule duration must be exactly 2 hours."}
            )
    ...
    def _validate_request_tutor_consistency(old_schedule, new_schedules):
        ...
        if len(tutor_ids) > 1:
            raise ValidationError(
                {
                    "detail": "All schedules in a request must be assigned to the same tutor."
                }
            )

    @staticmethod
    def _create_request(
        *, old_schedule, new_schedules, request_type, description, extension=None
    ):
    ...
    def _create_extension_request_schedules(cls, old_schedule, next_end_datetime):
        ...
        if extension_delta % cls.SESSION_DURATION != timedelta(0):
            raise ValidationError(
                {"end_datetime": "Schedule extension must be in multiples of 2 hours."}
            )
    ...
    def list(self, request, *args, **kwargs):
        ...
        if sort_by not in {"start_datetime", "end_datetime", "status"}:
            raise ValidationError(
                {
                    "sort_by": "Invalid sort field. Allowed values are start_datetime, end_datetime, status."
                }
            )
    ...
    def calendar_pagination(self, request):
        ...
        if mode not in {"month", "week"}:
            raise ValidationError(
                {"mode": "Invalid calendar mode. Allowed values are month, week."}
            )

        cursor_date = (
            parse_date(cursor_date_param) if cursor_date_param else timezone.localdate()
        )
        if not cursor_date:
            raise ValidationError(
                {"cursor_date": "Invalid date format. Use YYYY-MM-DD."}
            )
    ...
    def get_queryset(self):
        ...
        if start_date_param:
            ...
            if not start_date:
                raise ValidationError(
                    {
                        "start_date": "Invalid date format. Use YYYY-MM-DD or ISO datetime."
                    }
                )
            queryset = queryset.filter(
                start_datetime__gte=self._local_day_start(start_date)
            )
        ...
        if end_date_param:
            ...
            if not end_date:
                raise ValidationError(
                    {"end_date": "Invalid date format. Use YYYY-MM-DD or ISO datetime."}
                )
            queryset = queryset.filter(
                start_datetime__lt=self._local_day_start(end_date + timedelta(days=1))
            )

        status_values = self._parse_status_filters(raw_status_values)
        if status_values:
            invalid_status_values = sorted(
                set(status_values) - self.ALLOWED_SCHEDULE_STATUS
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        self._validate_not_past(
            start_datetime, "Schedules cannot be created in the past."
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if is_tutor(request.user):
            if not partial:
                raise ValidationError(
                    {
                        "detail": "Tutors can only submit partial updates for rescheduling."
                    }
                )

            if instance.status not in self.TUTOR_RESCHEDULABLE_STATUS:
                raise ValidationError(
                    {"detail": "Only upcoming or pending schedules can be rescheduled."}
                )

            payload_keys = set(request.data.keys())
            invalid_fields = payload_keys - {
                "start_datetime",
                "end_datetime",
                "description",
            }
            if invalid_fields:
                raise ValidationError(
                    {
                        "detail": "Tutors can only edit start_datetime, end_datetime, and description."
                    }
                )

            if (
                "start_datetime" not in request.data
                or "end_datetime" not in request.data
            ):
                raise ValidationError(
                    {
                        "detail": "start_datetime and end_datetime are required for tutor rescheduling."
                    }
                )
            ...
            if not request_description:
                raise ValidationError(
                    {"description": "Description is required for tutor requests."}
                )
            ...
            if (
                ...
                raise ValidationError(
                    {
                        "start_datetime": "The new schedule window must be different from the old one."
                    }
                )

            self._validate_not_past(
                next_start_datetime, "Tutors cannot reschedule to the past."
            )
            ...
            if pending_change_exists:
                raise ValidationError(
                    {
                        "detail": "A pending schedule change request already exists for this schedule."
                    }
                )

            same_start_datetime = next_start_datetime == instance.start_datetime
            if same_start_datetime:
                new_schedules = self._create_extension_request_schedules(
                    instance, next_end_datetime
                )
        ...
        next_start_datetime = serializer.validated_data.get(
            "start_datetime", instance.start_datetime
        )
        next_end_datetime = serializer.validated_data.get(
        ...
        if (
            ...
            self._validate_not_past(
                next_start_datetime, "Admins cannot reschedule to the past."
            )
            ...
            if same_start_datetime:
                ...
                created_schedules = self._create_extension_request_schedules(
                    instance, next_end_datetime
                )
    ...
    def request_schedule(self, request):
        if not is_tutor(request.user):
            return Response(
                {"detail": "Only tutors can request schedules."},
        ...
        self._validate_schedule_window(start_datetime, end_datetime)
        self._validate_not_past(
            start_datetime, "Tutors cannot request schedules in the past."
        )

        try:
            student = Student.objects.get(
                id=request_serializer.validated_data["student"]
            )
        except Student.DoesNotExist as exc:
            raise ValidationError(
                {"student": "Student with this id does not exist."}
            ) from exc
        ...
        if overlap_exists:
            raise ValidationError(
                {
                    "detail": "You already have an overlapping active/pending schedule for this student."
                }
            )
    ...
    def request_cancel(self, request, pk=None):
        if not is_tutor(request.user):
            return Response(
                {"detail": "Only tutors can request cancellations."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if schedule.tutor_id != request.user.id:
            return Response(
                {
                    "detail": "Tutors can only request cancellation for their own schedules."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if schedule.status not in {Schedule.STATUS_UPCOMING, Schedule.STATUS_PENDING}:
            raise ValidationError(
                {"detail": "Only upcoming or pending schedules can be cancelled."}
            )
        ...
        if pending_cancel_exists:
            raise ValidationError(
                {
                    "detail": "A pending cancellation request already exists for this schedule."
                }
            )
        ...
        request=inline_serializer(
            ...
            fields={
                "month": serializers.CharField(
                    help_text="Target month in YYYY-MM format.", required=True
                ),
            },
        ),
        responses={
    ...
    def generate_monthly_report(self, request):
        ...
        month_start = timezone.make_aware(
            datetime(target_month.year, target_month.month, 1),
            timezone.get_current_timezone(),
        )
        if target_month.month == 12:
            next_month_start = timezone.make_aware(
                datetime(target_month.year + 1, 1, 1), timezone.get_current_timezone()
            )
        else:
            next_month_start = timezone.make_aware(
                datetime(target_month.year, target_month.month + 1, 1),
                timezone.get_current_timezone(),
            )

        schedules = (
            Schedule.objects.select_related(
                "tutor", "student", "check_in", "check_in__check_out"
            )
            .filter(
                start_datetime__gte=month_start, start_datetime__lt=next_month_start
            )
        ...
        for student in Student.objects.filter(
            id__in=schedules.values_list("student", flat=True)
        ...
        for schedule in schedules:
            ...
            writer.writerow(
                [
                    schedule.id,
                    timezone.localtime(schedule.start_datetime).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    timezone.localtime(schedule.end_datetime).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    tutor_name,
                    ...
                    schedule.status,
                    (
                        timezone.localtime(check_in.check_in_time).strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        if check_in and check_in.check_in_time
                        else ""
                    ),
                    self._to_hyperlink_formula(
                        (
                            build_location_search_url(check_in.check_in_location)
                            if check_in
                            else ""
                        ),
                        "Open Location",
                    ...
                    self._to_hyperlink_formula(
                        (
                            ...
                            if check_in and check_in.check_in_photo
                            else ""
                        ),
                        "View Check In Photo",
                    ),
                    (
                        timezone.localtime(check_out.check_out_time).strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        if check_out and check_out.check_out_time
                        else ""
                    ),
                    self._to_hyperlink_formula(
                        (
                            check_out.check_out_photo
                            if check_out and check_out.check_out_photo
                            else ""
                        ),
                        "View Check Out Photo",
                    ),
                    (
                        str(check_out.total_shift_time)
                        if check_out and check_out.total_shift_time
                        else ""
                    ),
                ]
            )
        ...
        try:
            ...
            uploader.format_monthly_report_sheet(
                ...
                status_values=[
                    status_value for status_value, _ in Schedule.STATUS_CHOICES
                ],
            )
    ...
    def email_blast(self, request):
        ...
        try:
            ...
            return Response(
                {"detail": f"Failed to write audit logs to Google Drive: {exc}"},
                status=status.HTTP_502_BAD_GATEWAY,
            )
