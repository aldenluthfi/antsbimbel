...
class RequestViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def _with_effective_schedule_annotations(queryset):
        return queryset.annotate(
            effective_start_datetime=Coalesce(
                Min("new_schedules__start_datetime"), "old_schedule__start_datetime"
            ),
            effective_end_datetime=Coalesce(
                Max("new_schedules__end_datetime"), "old_schedule__end_datetime"
            ),
        )
    ...
    def list(self, request, *args, **kwargs):
        queryset = self._with_effective_schedule_annotations(
    ...
    def get_queryset(self):
        ...
        if is_tutor(user):
            queryset = queryset.filter(
                Q(new_schedules__tutor=user) | Q(old_schedule__tutor=user)
            )
        ...
        if tutor:
            queryset = queryset.filter(
                Q(new_schedules__tutor=tutor) | Q(old_schedule__tutor=tutor)
            )

        if student:
            queryset = queryset.filter(
                Q(new_schedules__student=student) | Q(old_schedule__student=student)
            )
        ...
        if start_date_param:
            ...
            queryset = queryset.filter(
                ...
                | (
                    Q(new_schedules__isnull=True)
                    & Q(old_schedule__start_datetime__gte=day_start)
                )
            )
        ...
        if end_date_param:
            ...
            queryset = queryset.filter(
                ...
                | (
                    Q(new_schedules__isnull=True)
                    & Q(old_schedule__start_datetime__lt=next_day_start)
                )
            )
        ...
        if status_values:
            invalid_status_values = sorted(
                set(status_values) - self.ALLOWED_REQUEST_STATUS
            )
    ...
    def approve(self, request, pk=None):
        ...
        if request_obj.request_type == Request.TYPE_CANCEL:
            if not old_schedule:
                raise ValidationError(
                    {"detail": "Cancel requests require an old schedule."}
                )
        ...
        if request_obj.request_type in {
            Request.TYPE_EXTENSION,
            Request.TYPE_RESCHEDULE,
            Request.TYPE_NEW_SCHEDULE,
        }:
            if not new_schedules:
                raise ValidationError(
                    {"detail": "Request does not have new schedules."}
                )
            ...
            if request_obj.request_type == Request.TYPE_NEW_SCHEDULE:
                ...
                pending_new_schedules = [
                    s for s in new_schedules if s.status == Schedule.STATUS_PENDING
                ]
            ...
            new_schedules = list(
                request_obj.new_schedules.select_related("check_in__check_out").all()
            )
            for schedule in new_schedules:
                if schedule.status == Schedule.STATUS_RESCHEDULED:
                    continue
                ...
                if has_checkout:
                    ...
                    if reschedule_qs.filter(status=Request.STATUS_PENDING).exists():
                        ...
                        reschedule_qs.filter(status=Request.STATUS_RESOLVED)
                        .exclude(
